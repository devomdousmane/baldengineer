...
/* ── Formatting ───────────────────────────────────────────────── */
/** Sérialisable (donc passable depuis un Server Component) — pas de fonction en prop. */
export type KpiFormat = "currency" | "percent";

function formatDisplay(n: number, format?: KpiFormat, currency = "EUR"): string {
  if (format === "currency") {
...
/* ── Animated counter ─────────────────────────────────────────── */
function AnimatedCounter({ to, format, currency }: { to: number; format?: KpiFormat; currency?: string }) {
  ...
  return <>{formatDisplay(display, format, currency)}</>;
}
...
interface KpiCardProps {
  ...
  accentColor?: string;
  sparkData?: number[];
  /** Format d'affichage du compteur animé — sérialisable, utilisable depuis un Server Component. */
  format?: KpiFormat;
  /** Devise à utiliser quand format="currency" (par défaut EUR). */
  currency?: string;
  index?: number;
}
...
export function KpiCard({
  ...
  sparkData, format, currency, index = 0,
}: KpiCardProps) {
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-2xl font-semibold text-[var(--color-text)] leading-none tabular-nums">
            {typeof rawValue === "number"
              ? <AnimatedCounter to={rawValue} format={format} currency={currency} />
              : value
            }
          </p>
