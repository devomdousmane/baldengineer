...
export function Sidebar({ userName, userAvatar, market, onSignOut, onTourStart }: SidebarProps) {
    ...
    return (
      <>
        ...
        >
          ...
          >
            {isCollapsed ? (
              <span className="w-6 h-6 rounded-[var(--radius-sm)] bg-white/10 flex items-center justify-center text-white text-xs font-bold shrink-0">
                B
              </span>
            ) : (
              <Image
        ...
        {/* Nav items */}
        <nav className="flex-1 overflow-y-auto overflow-x-hidden py-2 px-2" role="navigation">
          <motion.ul
        ...
        >
          ...
          <button
            ...
            className={`hidden lg:flex w-full items-center gap-2.5 px-3 py-2 rounded-[var(--radius-md)] text-sm text-[var(--color-text-2)] hover:bg-[var(--color-bg-2)] hover:text-[var(--color-text)] transition-colors duration-[var(--dur-fast)] cursor-pointer ${isCollapsed ? "justify-center" : ""}`}
            aria-label={collapsed ? "Développer la sidebar" : "Réduire la sidebar"}
          >
            {collapsed ? <PanelLeftOpen className="w-4 h-4 shrink-0" strokeWidth={1.75} /> : <PanelLeftClose className="w-4 h-4 shrink-0" strokeWidth={1.75} />}
            {!isCollapsed && <span>Réduire</span>}
          </button>

          {isCollapsed ? (
            <Tooltip content="Retour au site" side="right">
              <a
                href={process.env.NEXT_PUBLIC_SITE_URL ?? "/"}
          ...
          {(() => {
            const aideLink = (
              <Link
                ...
                onClick={handleNavigate}
                data-tour="nav-aide"
                className={`flex items-center gap-2.5 px-3 py-2 rounded-[var(--radius-md)] text-sm transition-colors duration-[var(--dur-fast)] ${isCollapsed ? "justify-center" : ""} ${
            ...
            return isCollapsed ? <Tooltip content="Aide" side="right">{aideLink}</Tooltip> : aideLink;
          })()}
          ...
          {(() => {
            ...
            return isCollapsed ? <Tooltip content="Paramètres" side="right">{settingsLink}</Tooltip> : settingsLink;
          })()}
  ...
  return (
    <>
      {/* Desktop — toujours visible, largeur animée selon collapsed */}
      <motion.aside
        className="sidebar-surface hidden lg:flex flex-col fixed left-0 top-0 bottom-0 border-r border-[var(--color-border)] z-[var(--z-sidebar)] overflow-hidden"
      ...
      {/* Mobile/tablette — drawer, toujours en version étendue */}
      <AnimatePresence>
        {mobileOpen && (
          <>
            <motion.div
